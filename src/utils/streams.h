...
namespace Stream
{
    ...
    {
      public:
        // A function that is used to load more data from the underlying data source.
        // Should return the actual amount of bytes read. If it returns 0, it's assumed that no more data is available.
        // This function will never be copied, only moved.
        ...
        // A function that can be used to create a user-facing state string, which normally contains a byte offset, or a line&column pair.
        // Should be a stateful functor. When it's called, the stored state should be updated.
        // The returned reference should probably point to the stored state. It will be used immediately after the call (if at all).
        // Can be null, then an empty string will be used.
        using state_func_t = std::function<const State &(const uint8_t *, std::size_t)>;

        // Returns a byte offset.
        [[nodiscard]] static state_func_t Binary()
        {
            struct S : State
            {
                std::size_t offset = 0;
            ...
            return [state = S{}](const uint8_t *, std::size_t size) mutable -> const State &
            {
                state.offset += size;
                return state;
            };
        ...
        [[nodiscard]] static state_func_t Text(Encoding enc = utf8)
        {
            struct S : State
            {
                int line = 1;
            ...
            {
              case ascii:
                return [state = S{}](const uint8_t *ptr, std::size_t size) mutable -> const State &
                {
      ...
      private:
        struct Data
        {
            ...
            // Updates and returns the current user-facing state string of the stream.
            // Can be null, then you'll get an empty string.
            state_func_t update_state = 0;
        };
        Data data;
        ...
        void FillBufferWithNewData()
        {
            ...
            if (data.buffer_pos > 0) // Don't attempt reading if EOF is reached.
            {
                if (data.update_state)
                    data.update_state(data.buffer, data.buffer_size);
      ...
      public:
        ...
        Input(func_t obtain_bytes, state_func_t update_state = 0, buffer_capacity_t buffer_capacity = default_buffer_capacity)
        {
            ...
            data.update_state = std::move(update_state);
        }

        // Constructs a stream referring to memory.
        Input(std::size_t size, const std::uint8_t *ptr, state_func_t update_state = 0)
        {
        ...
        Input(std::size_t size, const char *ptr, state_func_t update_state = 0)
            : Input(size, reinterpret_cast<const std::uint8_t *>(ptr), std::move(update_state))
        {}

        // Constructs a stream referring to a file.
        Input(std::string file_name, state_func_t update_state = 0, buffer_capacity_t buffer_capacity = default_buffer_capacity)
        {
            ...
            *this = Input(Meta::fake_copyable([handle = std::move(handle), file_name = std::move(file_name)](std::uint8_t *dest, std::size_t size) -> std::size_t
            {
                ...
                return result;
            }), std::move(update_state), buffer_capacity);
        }

        // Constructs a stream referring to a file.
        // An alternative overload, using a C-string.
        Input(const char *file_name, state_func_t update_state = 0, buffer_capacity_t buffer_capacity = default_buffer_capacity)
            : Input(std::string(file_name), std::move(update_state), buffer_capacity)
        {}
        ...
        // Returns false for a default-construted stream.
        bool WasNotAccessed() const
        {
            return data.buffer_pos > data.buffer_size;
        }

        // Updates the state function.
        // Does nothing if the function is already set, or if `Read*()` was called at least once on this stream.
        void SetStateFunc(state_func_t update_state)
        {
            if (WasNotAccessed() && !data.update_state)
                data.update_state = std::move(update_state);
        }

        // Returns a user-friendly state string. Normally it's a byte offset, or a line and a column.
        ...
        std::string GetStateString() const
        {
            ...
            return data.update_state(data.buffer, data.buffer_size).ToString();
        }

        // Returns a prefix that should be appended to exception messages.
        // Based on `GetStateString()`.
